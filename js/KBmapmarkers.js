...
function Map(name) {
  ...
  this.container = jQuery(
    "#" + this.name + " .KBmap__mapContainer .KBmap__mapHolder"
  );
  this.mapDataJSON;
  ...
  this.addMarker = function (icon, cordX, cordY, name) {
    ...
    this.mapMarkers[markerName] = new MapMarker(
      markerName,
      icon,
      cordX,
      cordY,
  ...
  this.importJSON = function (mapDataJSON) {
    ...
    for (importedMarkerName in this.mapDataJSON) {
      // add new marker
      this.addMarker(
        this.mapDataJSON[importedMarkerName].icon,
        this.mapDataJSON[importedMarkerName].cordX,
        this.mapDataJSON[importedMarkerName].cordY,
        importedMarkerName
      );

      // add modal to new added marker if specified in json
      if (
        this.mapDataJSON[importedMarkerName]["modal"] &&
        this.mapDataJSON[importedMarkerName]["modal"].title &&
        this.mapDataJSON[importedMarkerName]["modal"].content
      ) {
        this.mapMarkers[importedMarkerName].addModal(
          this.mapDataJSON[importedMarkerName]["modal"].title,
          this.mapDataJSON[importedMarkerName]["modal"].content
        );
      }
...
function MapMarker(name, icon, cordX, cordY, map) {
  ...
  this.setCurrent = function () {
    jQuery('[data-marker-name="' + this.name + '"]').css(
      "z-index",
      this.map.maxZindex
    );
    this.map.maxZindex++;
  ...
  this.generateMarker = function () {
    output =
      '<div class="KBmap__marker" data-marker-name="' +
      this.name +
      '" style="left: ' +
      this.cordX +
      "%; top: " +
...
function MarkerModal(modalTitle, content, linkedMapMarker) {
  ...
  this.generateModal = function () {
    output =
      '<div  class="KBmap__markerContent"><div class="KBmap__markerClose"><i class="fas fa-times"></i>		</div><h3 class="KBmap__markerTitle">' +
      this.title +
      "</h3>";

    output +=
      '<div class="KBmap__markerContentItem">' + this.content + "</div>";
  ...
  this.isModalActive = function () {
    return jQuery(
      '[data-marker-name="' + this.linkedMapMarker.name + '"]'
    ).hasClass("active");
  };
  ...
  this.openModal = function () {
    ...
    jQuery('[data-marker-name="' + this.linkedMapMarker.name + '"]').append(
      this.generateModal()
    );
  ...
  this.closeAllModals = function () {
    // console.log(this.linkedMapMarker.map.openedModals.length);
    if (this.linkedMapMarker.map.openedModals.length >= 2) {
      for (var i = 0; i < this.linkedMapMarker.map.openedModals.length; i++) {
        this.linkedMapMarker.map.openedModals[i].closeModal();
      }
    }
  };
  ...
  this.clearPosition = function () {
    $markerContentWidth = jQuery(
      '[data-marker-name="' + this.linkedMapMarker.name + '"]'
    )
      .find(".KBmap__markerContent")
      .outerWidth();
    $markerContentHeight = jQuery(
      '[data-marker-name="' + this.linkedMapMarker.name + '"]'
    )
      .find(".KBmap__markerContent")
      .outerHeight();
  ...
  function whileOffScreen() {
    ...
    while (
      ($positionedElem.offset().left < 0) &
      !(
        $positionedElem.offset().left + $markerContentWidth >
...
function generateUniqueMarkerName(map, name) {
  ...
  while (map.mapMarkers[objname]) {
    ...
    if (infiniteLoopCheck > 1000) {
      console.error(
        "After 10000 tries couldnt generate unique name for MapMarker object. Change max number in MapMarker object name [function generateName()]. Default max: 1000"
      );
      return false;
    }
  }
...
jQuery(document).ready(function () {
  ...
  jQuery("body").on("click", ".KBmap__marker img", function () {
    ...
    var clickedMarkerMapName = jQuery(this)
      .parent()
      .parent()
      .parent()
      .parent()
    ...
    jQuery.event.trigger(
      "markerClick",
      getKBmap(clickedMarkerMapName).mapMarkers[clickedMarkerName]
    );
  });
  ...
  jQuery("body").on("click", ".KBmap__markerClose", function (event) {
    ...
    var clickedMarkerName = jQuery(this)
      .parent()
      .parent()
      .attr("data-marker-name");
    var clickedMarkerMapName = jQuery(this)
      .parent()
      .parent()
      .parent()
  ...
  jQuery("body").on("click", ".KBmap__markerContent", function () {
    ...
    var mapMarkerMapParent = jQuery(this)
      .parent()
      .parent()
      .parent()
      .parent()
...
function createKBmap(name, mapsrc, mapDataJSON) {
  ...
  var output =
    '<div class="KBmap__mapContainer"><div class="KBmap__mapHolder"><img src="' +
    mapImg +
    '" alt="mapa"></div></div>';
